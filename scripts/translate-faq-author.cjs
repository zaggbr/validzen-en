...
async function main() {
  ...
  for (const post of posts) {
    ...
    // Check if FAQ needs translation (contains PT characters or just is PT)
    if (post.faq && post.faq.length > 0) {
      // Check if it's already English by checking a few words? 
      // Actually safer to just translate if it "looks" like PT or we haven't done it.
      // For simplicity, let's just translate if we haven't already.
